...
export function Footer({ contact }: { contact?: FooterContact }) {
    ...
    return (
        <footer className="bg-slate-900 text-slate-300 pt-12 mt-auto border-t border-slate-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-2xl font-black text-white tracking-tighter">
                            ...
                            <span className="block text-sm font-medium text-slate-400 mt-1">{cfg.nameFa}</span>
                        </h3>
                        ...
                        <div className="flex items-start gap-2.5 text-sm bg-slate-800/50 border border-slate-700/50 rounded-lg p-3">
                            <ShieldCheck className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
                            <span className="leading-6 text-slate-300">{cfg.warrantyNote}</span>
                        </div>
                        {(cfg.telegram || cfg.instagram) && (
                            <div className="flex items-center gap-2 pt-1">
                                {cfg.telegram && (
                                    <a href={cfg.telegram} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-lg bg-slate-800 hover:bg-primary flex items-center justify-center transition-colors" title="تلگرام">
                                        <Send className="w-4 h-4" />
                                    </a>
                                )}
                                {cfg.instagram && (
                                    <a href={cfg.instagram} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-lg bg-slate-800 hover:bg-primary flex items-center justify-center transition-colors" title="اینستاگرام">
                    ...
                    {/* دسترسی سریع */}
                    <div className="space-y-4">
                        <h4 className="text-lg font-bold text-white border-r-4 border-primary pr-3">دسترسی سریع</h4>
                    ...
                    {/* اطلاعات فروشگاه */}
                    <div className="space-y-4">
                        <h4 className="text-lg font-bold text-white border-r-4 border-primary pr-3">اطلاعات فروشگاه</h4>
                        <ul className="space-y-4 text-sm">
                            ...
                            <li className="flex items-center gap-3">
                                ...
                                <a href={`tel:${cfg.phoneRaw}`} className="font-bold text-lg text-white hover:text-primary transition-colors" dir="ltr">{cfg.phoneDisplay}</a>
                            </li>
                            <li className="flex items-center gap-3">
            ...
            <div className="bg-black/30 py-4 text-center text-xs text-slate-600 border-t border-white/5">
